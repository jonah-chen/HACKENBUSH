...
public:
	finite_generator(unsigned int seed,
					 float xz_radius = 10.0f, float y_min = 2.0f,
					 float y_max = 10.0f,
			...
			red_ratio(red_ratio)
	{
		...
		for (int i{}; i < air_edges; ++i)
		{
			...
			while (node1 == node2)
			{
				node1 = num_ground_nodes +
						rng() % (num_nodes - num_ground_nodes);
				node2 = num_ground_nodes +
						rng() % (num_nodes - num_ground_nodes);
			}
...
int main(int argc, char **argv)
{
	...
	if (argc == 2)
	{
		...
		if (out_file.is_open())
		{
			finite_generator(seed) << out_file;
			exit(0);
		}
	}
	...
	if (out_file.is_open())
	{
		...
		std::cout << "Ground Density Ratio: " << ground_density_ratio
				  << std::endl;
		...
		finite_generator(seed, xz_radius, y_min, y_max, grounded_nodes,
						 total_nodes, density,
						 ground_density_ratio, blue_ratio, red_ratio,
						 node_noise, density_noise) << out_file;
		exit(0);
	}
	else
	{
